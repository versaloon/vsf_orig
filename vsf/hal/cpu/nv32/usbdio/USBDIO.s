...
#define DPDM_MASK					((1 << VSFHAL_USBDIO_DP) | (1 << VSFHAL_USBDIO_DM))
...
USBDIO_DP_Handler
		...
		MOV		R1, SP
		SUBS	R1, R1, #1
		// R2, R3: uint32_t sample0 = (1 << VSFHAL_USBDIO_DM), sample1;
		MOVS	R2, #1
		LSLS	R2, R2, #VSFHAL_USBDIO_DM
#ifdef DEBUG
		...
		LDR		R4, =GPIO_IN_REG
#endif
		// R5: uint32_t mask = (1 << VSFHAL_USBDIO_DP) | (1 << VSFHAL_USBDIO_DM);
		LDR		R5, =DPDM_MASK
...
BIT0_OK_48
		...
		LSLS	R3, R3, #(31 - VSFHAL_USBDIO_DP)// [7 : 1]
...
BIT1_OK_48
		...
		LSLS	R2, R2, #(31 - VSFHAL_USBDIO_DP)
		BPL		BIT1_SAMPLE_0
		// sample 1: byte |= 0x02;
		ORRS	R6, R6, R0
		B		BIT1_BITSTUFFING
...
BIT2_OK_48
		...
		EORS	R3, R3, R2
		// if (sample1 & (1 << VSFHAL_USBDIO_DP))
		LSLS	R3, R3, #(31 - VSFHAL_USBDIO_DP)
		BPL		BIT2_SAMPLE_0
		// sample 1: byte |= 0x04;
		ORRS	R6, R6, R0
		B		BIT2_BITSTUFFING
...
TX_SE0
		...
		MOVS	R7, #1							// [3 : 1]
		LSLS	R4, R7, #VSFHAL_USBDIO_DP		// [4 : 1]
		NOP										// [5 : 1]
		LDRB	R7, [R0]						// [6 : 2]
		LDRB	R7, [R0]						// [8 : 2]
		LDRB	R7, [R0]						// [10: 2]
