...
struct interface_usbd_callback_t vsfsdcd_callback;
...
static vsf_err_t vsfsdcd_ep_set_type(uint8_t idx, enum interface_usbd_eptype_t type)
{
	return VSFERR_NONE;
}
...
const struct interface_usbd_t vsfsdcd_usbd =
{
	.init = vsfsdcd_init,
	.fini = vsfsdcd_fini,
	.poll = vsfsdcd_poll,
